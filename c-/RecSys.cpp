...
float s[19835][19835] = { 1.1 };

int _tmain(int argc, _TCHAR* argv[])
{
	fstream fin("train.txt");
		...
		for (int i = 0; i < num; i++)
		{
			...
			for (int j = 0; j <10; j++)
			{
				...
				for (int k = 0; k < border; k++)
				{
					...
					{
						recScore += user[uid].top[j].score * (user[index].is[k].score + user[index].mean);
						temps[j] = user[index].is[k].score + user[index].mean;
						break;
					}
					if (item < user[index].is[k].item)
					{
			...
			//+ user[uid].mean
			recScore = recScore / totalScore ;
			fout << item << "	" << recScore << endl;

			for (int j = 1; j < 6; j++)
			{
				rmse += (recScore - temps[j]) * (recScore - temps[j]);
			}
